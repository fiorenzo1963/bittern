...
/*!
 * if bi_status == 0, complete requests which have gennum <= gennum.
 * if bi_status != 0, complete all requests as it may not be possible to issue
 * an explicit flush.
 */
		...
		list_for_each_entry(wi,
			...
			if (wi->devio_gennum <= gennum) {
				...
				BT_TRACE(BT_LEVEL_TRACE1,
					 ...
					 "bi_sector=%lu, gennum=%llu/%llu, flush wait done, bi_status=%d",
				...
				cached_dev_make_request_endio(wi, bio, bi_status);
...
static void cached_devio_flush_end_bio(struct bio *bio)
{
	unsigned long flags;
	...
	BT_TRACE(BT_LEVEL_TRACE1, flush_meta->bc, NULL, NULL, bio, NULL,
		 "ack up to delayed flush gennum = %llu, bi_status=%d",
		 flush_meta->gennum,
		 bio->bi_status);
...
#warning "add error injection here when done merging with error handling"
	if (flush_meta == NULL) {
		 ...
		 */
		cached_devio_flush_end_bio_process(bc, 0, BLK_STS_RESOURCE);
		return;
	}
	flush_meta->bc = bc;
...
#warning "add error injection here when done merging with error handling"
	if (bio == NULL) {
		 ...
		 * Allocation failed, bubble up error to state machine.
		 */
		cached_devio_flush_end_bio_process(bc, 1, BLK_STS_RESOURCE);
		return;
	}

	/*
	 * set REQ_PREFLUSH|REQ_FUA if enabled
	 */
	...
	bio_set_dev(bio, bc->devio.dm_dev->bdev);
...
static void cached_devio_make_request_end_bio(struct bio *bio)
{
	struct bittern_cache *bc;
	...
	if (bio_data_dir(bio) == READ) {
		 ...
		 */
		cached_dev_make_request_endio(wi, bio, bio->bi_status);
		return;
	}
	 ...
	 */
	if (bio->bi_status != 0 || (wi->devio_flags & (REQ_PREFLUSH | REQ_FUA)) != 0) {
		/*
		 * ack previously pending writes, then ack current work_item.
		 */
		BT_TRACE(BT_LEVEL_TRACE1, bc, NULL, NULL, bio, NULL,
			 "endbio: write+flush bi_sector=%lu, gennum=%llu done, bi_status=%d",
			     bio->bi_iter.bi_sector,
			 wi->devio_gennum,
			 bio->bi_status);
		 ...
		 */
		cached_devio_flush_end_bio_process(bc, wi->devio_gennum, bio->bi_status);
			       ...
			       struct bio *bio)
{
	...
	if (bio_data_dir(bio) == WRITE) {
		...
		if ((wi->devio_gennum - bc->devio.gennum_flush) >
			 ...
			 * Issue flush
			 */
			bio_set_attrs_keep_op(bio, REQ_PREFLUSH | REQ_FUA);
			BT_TRACE(BT_LEVEL_TRACE1, bc, NULL, NULL, bio, NULL,
	...
	/* yiiiikesssss, need to code this better */
	wi->devio_flags = bio->bi_opf & (~REQ_OP_MASK);
